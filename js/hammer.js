...
manager.on('panmove', function(e) {
  ...
  if(touch.press) {
    ...
    // hammer 이동 비율 조정
    var dX = touch.current.x + (e.deltaX * 0.5);
    var dY = touch.current.y + (e.deltaY * 0.5);
...
manager.on('panend', function(e) {
  ...
  if(touch.isBoundary) touch.isBoundary = false;
    ...
    if(touch.press) {
      ...
      touch.current.x = touch.current.x + e.deltaX * 0.5;
      touch.current.y = touch.current.y + e.deltaY * 0.5;
    }
  }
});
